...
describe('Test order endpoint', () => {
    ...
    it('test update one endpoint', async () => {
        const response = await request.put('/api/order').set('content-type', 'application/json')
        .set('authorization', `Bearer ${JWT}`)
        .send({
            id: 1,
            OrderStatus: 2,
    ...
    it('test delete one endpoint', async () => {
        const response = await request.delete(`/api/order/1`).set('content-type', 'application/json')
        .set('authorization', `Bearer ${JWT}`)
        expect(response.status).toBe(200)
    })
})
