...
describe('Test product endpoint', () => {
    ...
    it('test update one endpoint', async () => {
        const response = await request.put('/api/product').set('content-type', 'application/json')
        .set('authorization', `Bearer ${JWT}`)
        .send({
            id: 2,
    ...
    it('test delete one endpoint', async () => {
        const response = await request.delete(`/api/product/1`).set('content-type', 'application/json')
        .set('authorization', `Bearer ${JWT}`)
        expect(response.status).toBe(200)
    })
})
