...
export class UserModel {
    async create(user: User): Promise<User> {
        try {
            ...
            const result = await conn.query(sql, [user.userName, user.firstName, user.lastName, hash(user.password)])
            conn.release()
    ...
    async updateOne(user: User): Promise<User> {
        try {
            ...
            const result = await conn.query(sql, [user.id, user.userName, user.firstName, user.lastName, hash(user.password)])
            conn.release()
            return result.rows[0]
        } catch (error) {
